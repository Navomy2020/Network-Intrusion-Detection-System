...
SCALED_EXCLUSION_LIST_FOR_NANDANA = [
    'protocol',
...
try:
    ...
    print(f"Error loading model file: {MODEL_PATH} not found. Make sure the path is correct.", file=sys.stderr)
...
def preprocess_data(df_input_raw, fitted_scaler):
    ...
    numerical_cols_for_scaling = [col for col in FEATURE_NAMES if col not in SCALED_EXCLUSION_LIST_FOR_NANDANA]
